...
public class SlimeSimulation : MonoBehaviour
{
    ...
    {
        Init();
        transform.GetComponentInChildren<MeshRenderer>().material.SetTexture(Shader.PropertyToID("_BaseMap"), displayTexture);
    }

    void Init()
    {
        //Create render textures
        ComputeHelper.CreateRenderTexture(ref trailMap, settings.width, settings.height, filterMode, format, "Trail Map");
        ComputeHelper.CreateRenderTexture(ref diffusedTrailMap, settings.width, settings.height, filterMode, format, "Diffuse Trail Map");
        ComputeHelper.CreateRenderTexture(ref displayTexture, settings.width, settings.height, filterMode, format, "Display Texture");
