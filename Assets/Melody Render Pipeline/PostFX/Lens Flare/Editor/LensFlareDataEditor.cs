...
class LensFlareDataEditor : Editor
{
    ...
    void OnAdd(ReorderableList list)
    {
        ...
        // Set Default values
        (target as LensFlareData).elements[newIndex] = new LensFlareDataElement();
        serializedObject.Update();
    }
    ...
    void DrawListSize(Rect rect)
    {
        ...
        if (newCount != previousCount)
        {
            ...
            if (newCount > previousCount)
            {
                LensFlareData lensFlareData = target as LensFlareData;
                for (int i = previousCount; i < newCount; ++i)
                    lensFlareData.elements[i] = new LensFlareDataElement();
    ...
    {
        int line = 1; //Type
        switch (GetEnum<LensFlareType>(type))
        {
            case LensFlareType.Circle:
                line += 3;
                break;  //Gradient, Falloff, Invert
            case LensFlareType.Image:
                line += 2;
                break;  //Flare Texture, Use Aspect Ratio
            case LensFlareType.Polygon:
                line += 5;
    ...
    int GetMultipleElementsCathegoryLines(SerializedProperty enabled, SerializedProperty distribution)
    {
        ...
        switch (GetEnum<LensFlareDistribution>(distribution))
        {
            case LensFlareDistribution.Uniform:
                line += 1;
                break;  //UniformAngle
            case LensFlareDistribution.Curve:
                line += 3;
                break;  //Position Variation, Rotation, Scale
            case LensFlareDistribution.Random:
                line += 5;
    ...
    void ComputeThumbnail(ref Texture2D computedTexture, SerializedProperty element, LensFlareType type, int index)
    {
        ...
        if (allowMultipleElementProp.boolValue && countProp.intValue > 1)
        {
            ...
            {
                Vector4 flareData0 = LensFlareCommon.GetFlareData0(Vector2.zero, Vector2.zero, Vector2.one, Vector2.zero, rotationProp.floatValue + currentAngle, 0f, 0f, Vector2.zero, false);

                float cos0 = flareData0.x;
                float sin0 = flareData0.y;

                if (GetEnum<LensFlareDistribution>(distributionProp) == LensFlareDistribution.Random)
                    RandomRange(-1.0f, 1.0f); // Local Intensity

                Vector2 localLocalSize;
                if (GetEnum<LensFlareDistribution>(distributionProp) == LensFlareDistribution.Uniform)
                    localLocalSize = localSize;
                else if (GetEnum<LensFlareDistribution>(distributionProp) == LensFlareDistribution.Random)
                ...
                {
                    switch (GetEnum<LensFlareDistribution>(distributionProp))
                    {
                        case LensFlareDistribution.Uniform:
        ...
        {
            Vector4 flareData0 = LensFlareCommon.GetFlareData0(Vector2.zero, Vector2.zero, Vector2.one, Vector2.zero, rotationProp.floatValue, 0f, 0f, Vector2.zero, false);
    ...
    int GetElementHash(SerializedProperty element, LensFlareType type, int index)
    {
        SerializedProperty sizeXYProp = element.FindPropertyRelative("sizeXY");
    ...
    Texture2D GetCachedThumbnailProceduralTexture(SerializedProperty element, LensFlareType type, int index)
    {
    ...
    void DrawThumbnailProcedural(Rect rect, SerializedProperty element, LensFlareType type, int index)
    {
    ...
    bool DrawElementHeader(Rect headerRect, SerializedProperty isFoldOpened, bool selectedInList, SerializedProperty element)
    {
        ...
        texture2D.SetPixel(0, 0, Color.white * 0.5f);
        texture2D.Apply(false, true);
        // ellipsis menu
        if (GUI.Button(contextMenuRect,new GUIContent(texture2D, ""), new GUIStyle("IconButton")))
        {
        ...
        bool newVisibility = GUI.Toggle(visibilityRect, visible.boolValue, GUIContent.none, new GUIStyle("ShurikenToggle"));
    ...
    void DrawSummary(Rect summaryRect, SerializedProperty element, int index)
    {
        ...
        DrawThumbnailProcedural(thumbnailRect, element, GetEnum<LensFlareType>(type), index);
    ...
    void DrawTypeCathegory(ref Rect remainingRect, SerializedProperty element)
    {
        ...
        LensFlareType flareType = GetEnum<LensFlareType>(type);
        switch (flareType)
        {
            case LensFlareType.Image:
                {
            ...
            case LensFlareType.Polygon:
                {
                    ...
                    if (flareType == LensFlareType.Polygon)
                    {
